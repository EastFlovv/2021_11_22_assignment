function solution(newId) {
  ...
  str = (str.length > 15 ? str.substring(0, 15) : str).replace(/\.$/, '');;

  // 7단계 new_id의 길이가 2자 이하라면, new_id의 마지막 문자를 new_id의 길이가 3이 될 때까지 반복해서 끝에 붙입니다.
  str = str.length < 3 ? str.padEnd(3, str[str.length-1]) : str;
  return str;
}

// 정규 표현식을 이용하여 좀 더 클린 코드로 만들어 보기
